...
-- Q2.Which customer used a dicount but still spent more than the average purchase amount?
...
SELECT 
    ...
    CAST(ROUND(AVG(review_rating), 2) AS DECIMAL(10,2)) AS avg_rating
FROM customer
...
-- Q7.Segments customers into New,Returning, and Loyal based on their total number of previous purchases, and show the count of each segment.alter
...
with item_counts as (
select category,item_purchased ,count(customer_id) as total_final_orders,
...
select item_rank,category,item_purchased,total_final_orders from item_counts where item_rank <= 3;

-- Q9.Are customers who are repeat buyers (more than 5 previous purchases)also likely to subscribe?
select subscription_status,count(customer_id) as Repeated_customer_buyers from customer 
